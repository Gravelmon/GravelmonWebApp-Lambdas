import {
    createErrorResponse, createSuccessResponse, FormNode, FormData,
    GravelmonDynamoDBService,
    LambdaEvent,
    parseBody, PokemonData,
} from "gravelmon-dynamodb";

export const handler = async (event: LambdaEvent) => {
    if (!process.env.DYNAMODB_TABLE) {
        return createErrorResponse(500, "DYNAMODB_TABLE environment variable is required.");
    }
    const gravelmonDynamoDBService = new GravelmonDynamoDBService(process.env.DYNAMODB_TABLE);

    try {
        const parsed = parseBody<{
            pokemonData: PokemonData,
            formData: FormData
        }[]>(event);
        const pokemonNodes = parsed.map(entry => new FormNode(entry.pokemonData, entry.formData));
        let pokemonResults = await gravelmonDynamoDBService.batchPutItems(pokemonNodes) as FormNode[];

        return createSuccessResponse(200, {
            forms: pokemonResults
        })
    } catch (error: any) {
        console.log(error);
        return createErrorResponse(500, error.message || "Internal error");
    }
};
